...
function PurchasHistory() {
  ...
  const [PurchasHistoryData, setPurchasHistoryData] =
    useState<PurchaseHistoryDTO[]>();
          ...
          <div className="w-[92%] lgl:w-[74%] flex flex-col gap-6  lgl:my-10 mx-auto lgl:ml-5">
            <div className="w-full  bg-white py-10 dark:bg-slate-900">
              ...
              {PurchasHistoryData?.length === 0 ? (
                ...
                <>
                  {PurchasHistoryData &&
                    PurchasHistoryData.map((item) => {
                      return item.projects.map((project) => {
                        let totalAmount = 0;
                        return (
                          <>
                            <div className="p-0.5 dark:bg-slate-600 bg-yellow-50 h-28 rounded-md sm:w-full">
                              <img
                                className="mx-4 mt-6 rounded-full float-start w-28  "
                                src={project.thumbnail?.toString()}
                                alt="projectImage"
                              />
                              <a
                                className="text-blue-500 dark:text-blue-200 mt-6 text-xl font-extrabold line-clamp-1"
                                href={`/project/${project.projectId}`}
                              >
                                {project.projectName}
                              </a>

                              <span className="float-end mt-1 font-black mr-4 sm:w-auto">
                                贊助日期:{item.orderDate}
                              </span>
                              ...
                              {project.products!.map((product) => {
                                ...
                                return (
                                  <div
                                    ...
                                    className="border-b-[2px] border-b-gray-400 p-4 lg:flex gap-3 bg-slate-100 rounded-lg mt-1 my-2 dark:bg-slate-700 lg:h-24"
                                  >
                                    <div className="float-left w-64">
                                      <img
                                        className="mx-auto h-full w-56"
                                    ...
                                    <div className="w-4/5 flex flex-col -mt-5">
                                      <br />
                                      <h2 className="text-[20px] font-semibold -mt-3">
                                        {product.productName}
                                      </h2>
                                      <div className="float-end">
                                        ...
                                        <p className="text-sm float-start">
                                          品項單價:{product.productPrice}
                                        </p>
                                        <p className="float-end text-lg font-extrabold text-green-400">
                                          小計:$
                                          {(
...
前往結帳
                                ...
                                <span className="font-bold float-end mr-5 text-2xl">
                                  總計消費金額: NT$
                                  {item.donate != 0 && item.donate != null
                                    ? (totalAmount +
                                        item.donate -
                                        item.discount <
                                      0
                                    ...
                                    : (totalAmount - item.discount < 0
                                        ? 0
                                        : totalAmount - item.discount
                                      ).toLocaleString()}{" "}
                                </span>
                                {item.donate != 0 && item.donate != null ? (
                                  <span className="text-yellow-500 font-black mr-5 px-4 text-xl">
                                    加碼贊助:NT${item.donate}
                                  </span>
                                ) : (
                                  <></>
                                ...
                                {item.discount != 0 ? (
                                  <span className="text-green-400 font-black mr-5 text-xl">
                                    折價卷折抵:NT${item.discount}
                                  </span>
                                ) : (
                                  <></>
                                )}
